...
for(i in as.numeric(input$report_choices)){
  ...
  ## TIC and EIC
    ...
    axis(side = 1,at=seq(data.VarSel$range.time[1],data.VarSel$range.time[2],length.out = 10),
  ...
  ## Ion cluster
    ...
    df <- data.frame(x = as.numeric(rownames(data.VarSel$eic[i$index,])),y = apply(data.VarSel$eic[i$index,],1,sum))
      ...
      plot(df,type="h",xlim=if(!is.null(i$xlim)){i$xlim}else{range.mz_full()},ylab="intensity [AU]",xlab=expression(italic(m/z)),ylim=c(0,df[1,2]*1.2),main="Selected ion cluster")
  ...
  ## full scan at maximum
    ...
    df <- df[order(df[,2],decreasing = T),]
    # if(!is.null(i$xlim)){df = df[df[,1] > i$xlim[1] && df[,1] < i$xlim[2],]}
    plot(df,type="h",xlim=if(!is.null(i$xlim)){i$xlim}else{range.mz_full()},ylab="intensity [AU]",xlab=expression(italic(m/z)),ylim=c(0,df[1,2]*1.2),main="Full scan at cluster EIC maximum")
